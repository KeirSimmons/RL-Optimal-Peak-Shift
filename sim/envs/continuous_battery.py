...
"""
Simple battery definition which can handle a continuous
action space
"""

class ContinuousBattery(Interface):
    ...
    def __init__(self,
        ...
        self.action_space = Box(np.array([0]), np.array([0.1]))
    ...
    def step(self, action=None, energy=None):
        ...
        if energy is not None:
            ...
            to_buy = np.maximum(-1, np.minimum(1, action))
            to_buy = 0.03 * (to_buy + 1) / 2
        ...
        # For energy calculations, make sure the amount we buy is valid
        to_buy = np.maximum(minimum_energy_required, np.minimum(self.max_capacity - current_energy, to_buy))
